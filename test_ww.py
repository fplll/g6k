...
# - - - Now we test if we can restore the Siever object - - -
print("Testing restored Siever object")
...
if sieve_dim==n:
    ...
    projerr = np.array( G.to_canonical( G.from_canonical(e,start=n-sieve_dim), start=n-sieve_dim) )
    # print(f"projerrL {projerr}")
    # print(f"out: {out}")
    diff_v =  np.array(projerr)-np.array(out)
    # print(f"Diff btw. cvp and slicer: {diff_v}")
